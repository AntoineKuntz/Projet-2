...
// insertions de nouvelle information dans la bdd
    ...
    {
        $statement = $this->pdo->prepare("INSERT INTO " . self::TABLE . " (title , description , comment) VALUES (:title, :description, :comment)");
...
// mise a jours des informations présente dans la bdd
    ...
    {
        $statement = $this->pdo->prepare("UPDATE " . self::TABLE . " SET `title`= :title , `description` = :description ,`comment` = :comment WHERE id=:id");
